...
- All routes are currently open because `"/**"` is permitted
...
Two default users are configured with **no-op password encoding** (plain text). They are **not required** right now because all routes are open:
...
3. **Logout** → Available at `/doLogout` (session invalidated)
...
_Version: 3.3 (With Modular Auth + Public Routes)_  
