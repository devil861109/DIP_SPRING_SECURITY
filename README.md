...
- **JWT refresh tokens** - separate expiration for refresh tokens
...
- `/refresh` endpoint allows token refresh without re-login
...
**application.yml** - App configuration (uses environment variables with defaults)
```yaml
server:
...
spring:
  application:
    name: ${SPRING_APP_NAME:DIP_SPRING_SECURITY}
  datasource:
    url: ${SPRING_DATASOURCE_URL:jdbc:mariadb://localhost:3306/springsecurity}
    username: ${SPRING_DATASOURCE_USERNAME:root}
    password: ${SPRING_DATASOURCE_PASSWD:d1p10m4d0j4v4}
  jpa:
    hibernate:
      ddl-auto: ${SPRING_DDL_AUTO:none}

jwt:
  secret: ${SPRING_JWT_SECRET:5f39c83d2d0482e07b5306f3f5a0fd8e1b4e9844987121ba4b99f3f4805fd4f85...}
  expirationDateInMs: ${SPRING_JWT_EXP_DATE_MS:3600}
  refreshExpirationDateInMs: ${SPRING_JWT_REFRESH_EXP_DATE_MS:900}
```

**Note:** The application uses YAML configuration. Users are loaded from the database via `UserDetailsServiceImpl`.
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        http
            .authorizeHttpRequests((authz) -> authz
                ...
                .requestMatchers("/pokemon").hasAnyRole("ADMIN","USER")
                .requestMatchers("/rickmorty").hasAnyRole("ADMIN","USER")
                .requestMatchers("/pets/**").hasAnyRole("ADMIN","USER")
...
- Protected endpoints: `/pokemon`, `/rickmorty`, `/pets/**` require USER or ADMIN role
- Role checks for `/user` and `/admin` exist but are currently bypassed due to `"/**"`
...
- **Token Expiration**: Access tokens expire after `jwt.expirationDateInMs` (default: 3600 seconds = 1 hour)
- **Refresh Tokens**: Separate refresh token with `jwt.refreshExpirationDateInMs` (default: 900 seconds)
- **Refresh Endpoint**: `/refresh` endpoint allows clients to get a new token without re-login
...
|-----|---------|-----------------|-------|
| `/` | Home page | Open | `"/**"` is permitted |
| `/user` | User page | USER or ADMIN role | Role-protected page |
| `/admin` | Admin page | ADMIN role | Admin-only page |
| `/pokemon` | Pokemon API | USER or ADMIN role | Protected endpoint |
| `/rickmorty` | Rick & Morty API | USER or ADMIN role | Protected endpoint |
| `/pets/**` | Pets API | USER or ADMIN role | Protected endpoint |
| `/login` | Custom login page | Open | Form login endpoint |
| `/refresh` | Token refresh | Open | Refresh JWT without re-login |
| `/doLogout` | Custom logout | Any authenticated user | Clears JWT cookie |
| `/api/**` | REST endpoints | Open | Public API routes |
| `/auth/welcome` | REST endpoint | Open | Plain text response |
...
- Ensure `jwt.secret` is configured in `application.yml`
- Check `jwt.expirationDateInMs` setting
- Token may have expired; login again or use `/refresh` endpoint

### "Refresh token not working"
- Ensure `jwt.refreshExpirationDateInMs` is configured
- Refresh token must not be expired
- POST to `/refresh` with valid refresh token cookie
